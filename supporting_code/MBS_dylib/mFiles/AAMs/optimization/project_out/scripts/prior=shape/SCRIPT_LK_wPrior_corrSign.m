...
p_init = shape.b(1:num_p, last_frame);

% RE-WRITTEN USING Sj
s_init = s0_vec + Sj_vec(:,1:num_p)*p_init;
...
q_init = NaN(4,1);
q_init(1) = (T(1,1) - 1) * weights(1);
q_init(2) = T(2,1) * weights(2);
q_init(3) = s_pos_mean(1) * weights(3);
q_init(4) = s_pos_mean(2) * weights(4);
...
% user set parameters
reg_weight = 1e12;  % cost, i.e. scaling of regularization term
...
for t = 1:1%size(Trev_imageStack,3),
    ...
    % step frame #
    raw_image = Trev_imageStack(:,:,t);         % step image forward (actually, backward) one frame
    ...
    for i = 1:2%100,
        ...
        dFdp = c*sign(d);
        dppdDp = -1;                                % LINEAR APPROX FOR NOW
        
        % (6a)
        H_prior = zeros(5,5);
        H_prior(5,5) = (dFdp*dppdDp)^2;
        ...
        SD_update = SD_update - [0, 0, 0, 0, dFdp*dppdDp*F]';
...
fname = strcat('som7_c=1e6.avi');
writerObj = VideoWriter(fname);
open(writerObj);

fhandle = figure;

for t = 1:size(sp_save,3);
    ...
    plot(sp_save(1,:,t),sp_save(2,:,t),'wo')
...
for i = 1:size(q_save,1),
    
    plot(q_save(i,:)),
    print('-dtiff',strcat('q',num2str(i),'.tif'))
    
end
    
for i = 1:size(p_save,1),
    
    plot(p_save(i,:)),
    print('-dtiff',strcat('p',num2str(i),'.tif'))
