...
% user set parameters
reg_weights = 1e-3*[1e-1; 1e-1; 1e-1; 1e-1; 1e3; 0];
sigma = 1e-2;
...
for t = 1:size(Trev_imageStack,3),
    ...
    [q_final_save(:,t), p_final_save(:,t), L_final(:,t), sp_final_save(:,:,t), Err_final(:,t), rho_p_final(:,t), q, p, L, sp_save, conv_flag_save(t)] = LK_GradDescent_Sim_Rob_wPriors(raw_image,A0_vec,Grad_A0_vec,Ai_vec,Grad_Ai_vec,dNdq,dWdp,...
        s0,s0_vec,Sj,Sj_vec,Sja,Sja_vec,weights,SFP_positions,pixelList,q_init,p_init,L_init,conv_thresh,num_iters,reg_weights,step_size,sigma);
...
save_flag = 1;
fname = 'Robust=1e-2_priors=1e-3*[1; 1; 1e-1; 1e-1; 1e3; 0]_SIM.avi';
...
%% movie of error image

%
load('som7_model.mat','X');

%
save_flag = 1;
fname = 'ERRORIMAGE^2_Robust_priors=1e-3*[1; 1; 1e-1; 1e-1; 1e3; 0]_SIM.avi';
...
fhandle = figure;
for t = 1:size(Err_final,2),
    
    %
    [Err_im] = make_image_from_image_vector(Err_final(:,t).^2, X, pixelList);
...
%
save_flag = 1;
fname = 'rhoP_Robust_priors=1e-3*[1e-1; 1e-1; 1e-1; 1e-1; 1e2; 0]_SIM.avi';
...
for t = 1:size(Err_final,2),
    ...
    %
    [rho_p_im] = make_image_from_image_vector(rho_p_final(:,t), X, pixelList);
