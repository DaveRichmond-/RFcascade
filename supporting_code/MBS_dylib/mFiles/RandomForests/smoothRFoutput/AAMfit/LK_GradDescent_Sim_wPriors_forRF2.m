function [sp_final, cost] = LK_GradDescent_Sim_wPriors_forRF2(raw_image, modelSegmentsAAM, q_init, p_init, L_init, q_prior, p_prior, L_prior, LKparams)

% same as "*_forRF", but no arrays.  should be much faster...
...
%     [st1, s1] = LK_Warp(s0,Sj,Sja,weights,q(:,i),p(:,i));
    ...
    if sum((w_positions(2,:) < 1) + (w_positions(1,:) < 1) + (w_positions(2,:) > size(raw_image,1)) + (w_positions(1,:) > size(raw_image,2))) > 0,
        %
        if i == 1,
            [qp, pp, sp_final] = LK_ParamsOf_Comp_Warps(s0,Sj,Sja,weights,q,p,zeros(size(q,1),1),zeros(size(p,1),1));
            cost = 1/eps;
        else
            sp_final = sp;        
            cost = 1/eps;
...
%     Err_image_vec =  A0_vec + Ai_vec*L(:,i) - w_image_vec;         % note the change of sign from before!
    Err_image_vec =  A0_vec + Ai_vec*L - w_image_vec;         % note the change of sign from before!
    
    % compute the SD image
%     SD = LK_SDimage_Sim_FAST(Grad_A0_vec, dNdq, dWdp, Ai_vec, L(:,i), Grad_Ai_vec);
    SD = LK_SDimage_Sim_FAST(Grad_A0_vec, dNdq, dWdp, Ai_vec, L, Grad_Ai_vec);
...
%         d = [qp(:,i-1); pp(:,i-1); Lp(:,i-1)] - [q_prior; p_prior; L_prior];
        d = [q(:); p(:); L(:)] - [q_prior; p_prior; L_prior];
...
%         dppdDp(1:length(q_init)+length(p_init),1:length(q_init)+length(p_init)) = prior_reparameterization(s0,s0_vec,Sj,Sj_vec,Sja,Sja_vec,weights,q(:,i-1),p(:,i-1),dq(:,i-1),dp(:,i-1),qp(:,i-1),pp(:,i-1));
        dppdDp(1:length(q_init)+length(p_init),1:length(q_init)+length(p_init)) = prior_reparameterization(s0,s0_vec,Sj,Sj_vec,Sja,Sja_vec,weights,q_old,p_old,dq,dp,q,p);
...
%     dL(:,i) = dL(:,i).*step_size(4+size(p_init,1)+1:end);
    ...
    % CAREFUL OF DIMENSIONALITY
    dq = Delta(1:4).*step_size(1:4);
    dp = Delta(5:4+size(p_init,1)).*step_size(5:4+size(p_init,1));
    dL = Delta(4+size(p_init,1)+1:end).*step_size(4+size(p_init,1)+1:end);
