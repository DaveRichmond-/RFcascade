function [segmentsFit, costs] = fitAAMtoProbMap_gridSample(grayImage, probMap, modelCentroids, modelSegmentsAAM, numGDsteps, priorStrength, numOffsets, offsetScale, output_flag)
...
[p_init, q_init, Offsets] = initializeModelSegmentsAAM(centroids, modelCentroids, modelSegmentsAAM, numOffsets, offsetScale);
...
% initialize
costs = zeros(size(probMap,3)-1, numOffsets);
segmentsFit = zeros(2, 8, size(probMap,3)-1, numOffsets);

for i = 1:size(probMap,3)-1,
    
    for j = 1:numOffsets,
        
        dq = [0; 0; Offsets(i,1,j); Offsets(i,2,j)] .* modelSegmentsAAM.weights;
        [segmentsFit(:,:,i,j), costs(i,j)] = LK_GradDescent_Sim_wPriors_forRF(grayImage, modelSegmentsAAM, q_init(:,i)+dq, p_init(:,i), L_init(:,i), LKparams);
