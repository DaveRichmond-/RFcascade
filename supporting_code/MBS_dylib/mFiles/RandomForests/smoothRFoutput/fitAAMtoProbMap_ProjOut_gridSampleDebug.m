function [sp_save, q, p, L, cost] = fitAAMtoProbMap_ProjOut_gridSampleDebug(grayImage, probMap, modelCentroids, modelSegmentsAAM, numGDsteps, varargin);
...
LKparams.reg_weights = 1e-3*[1; 1; 1e-1; 1e-1; 1e3];
LKparams.step_size = 0.5*ones(5,1);
LKparams.num_iters = numGDsteps;
LKparams.conv_thresh = 2*[0.0005; 0.0005; 0.001; 0.001; 0.00005];
...
for i = i1:i2,
    ...
    for j = j1:j2,
        ...
        tic,
        [q_final_save(:,i), p_final_save(:,i), sp_final_save(:,:,i), q, p, sp_save, conv_flag_save(i), cost] = LK_GradDescent_wPriors_forRFDebug(grayImage, modelSegmentsAAM, q_init(:,i) + Offset, p_init(:,i), LKparams);
        toc,
        
    end
end

L = zeros(1,size(q_final_save,2));